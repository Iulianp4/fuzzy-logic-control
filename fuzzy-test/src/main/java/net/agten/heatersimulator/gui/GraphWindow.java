...
public class GraphWindow {
	...
	private JPanel createFUZZYPanel() {
		JPanel algorithmPanel = new JPanel();
		algorithmPanel.setLayout(new BoxLayout(algorithmPanel, BoxLayout.Y_AXIS));
		algorithmPanel.add(createSliderPanel("Current Temp:", params.getEnvironmentTemperature(), 0, 50,
				new ValueChangedAction() {
					@Override
					public void valueChanged(double newValue) {
						params.setEnvironmentTemperature((short)Math.round(newValue));
					}
				}));
		algorithmPanel.add(createSliderPanel("Target Temp:", params.getTargetTemperature(), 0, 300,
				new ValueChangedAction() {
					@Override
					public void valueChanged(double newValue) {
						params.setTargetTemperature((short)Math.round(newValue));
					}
				}));
		return algorithmPanel;
	}
